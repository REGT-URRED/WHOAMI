...
import { loadConfig } from '@whoami/core';
import { getBackend, listBackends } from '@whoami/core';
import { initDisplay, banner, primary, secondary, accent, success, error, warning, info, muted } from '@whoami/core';
import { recordRun, getAgentStats } from '@whoami/core';
import { startTui } from '@whoami/tui';
import { ProjectMemory } from '@whoami/rag-memory';
...
async function main() {
  ...
  const memoryCmd = program
    .command('memory')
    .description('Project RAG memory — vector search, stats, and reindex');

  memoryCmd
    .command('stats')
    .description('Show memory statistics for current project')
    .action(async () => {
      const mem = new ProjectMemory(process.cwd());
      await mem.init();
      const stats = mem.getStats();
      console.log(info('RAG Memory Statistics:'));
      console.log(muted(`  Project: ${stats.projectPath}`));
      console.log(muted(`  Embedder: ${primary(stats.embedder)}`));
      console.log(`  Files indexed: ${success(String(stats.files))}`);
      console.log(`  Decisions: ${accent(String(stats.decisions))}`);
      console.log(`  Error patterns: ${warning(String(stats.errors))}`);
      console.log(`  Sessions: ${info(String(stats.sessions))}`);
      console.log(muted(`  Last index: ${stats.lastIndex}`));
    });

  memoryCmd
    .command('search <query>')
    .description('Search the RAG memory by semantic similarity')
    .option('-k, --top <number>', 'Number of results', '5')
    .action(async (query: string, opts: { top: string }) => {
      const mem = new ProjectMemory(process.cwd());
      await mem.init();
      const results = await mem.search(query, parseInt(opts.top));
      if (results.size === 0) {
        console.log(muted('No results found. Try reindexing with: whoami memory reindex'));
        return;
      }
      for (const [collection, hits] of results) {
        console.log(info(`\n${collection}:`));
        for (const hit of hits) {
          const score = hit.score > 0.8 ? success(`${(hit.score * 100).toFixed(1)}%`) : 
                        hit.score > 0.5 ? primary(`${(hit.score * 100).toFixed(1)}%`) : 
                        `${(hit.score * 100).toFixed(1)}%`;
          console.log(`  [${score}] ${hit.content.slice(0, 120)}...`);
        }
      }
    });
  ...
  memoryCmd
    .command('reindex')
    .description('Re-scan and re-index all project files')
    .action(async () => {
      console.log(info('Reindexing project files…'));
      const mem = new ProjectMemory(process.cwd());
      await mem.init();
      await mem.indexProject();
      const stats = mem.getStats();
      console.log(success(`Reindex complete: ${stats.files} files indexed`));
    });

  memoryCmd
    .command('init')
    .description('Initialize RAG memory for this project')
    .option('-s, --strategy <strategy>', 'Embedding strategy: bow, tfidf, minilm')
    .action(async (opts: { strategy?: string }) => {
      console.log(info('Initializing RAG memory…'));
      const mem = new ProjectMemory(process.cwd());
      await mem.init(opts.strategy as any);
      await mem.indexProject();
      const stats = mem.getStats();
      console.log(success(`Memory initialized: ${stats.files} files indexed with ${stats.embedder}`));
      console.log(muted(`Location: ${process.cwd()}\\.whoami\\memory`));
    });
